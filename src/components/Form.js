        ...
        {({ errors, touched }) => (
          <StyledFormikForm>
            ...
            >
              ...
              <motion.div variants={buttonVariants} className="auth-redirect">{register ? (<p>Already a user? <button role="button" onClick={registerClick}>Login now</button></p>): (<p>don't have an account? <span onClick={registerClick} role="button">join now.</span></p>)}</motion.div>
            </motion.div>
          </StyledFormikForm>
        )}
      </Formik>
